...
class EstadisticaVIP:
    ...
    total_picks: int = 0
    picks_ganados: int = 0
    picks_perdidos: int = 0
    tasa_aciertos: float = 0.0
    roi_total: float = 0.0
    ligas_monitoreadas: int = 0
    partidos_hoy: int = 0
    partidos_en_vivo: int = 0

# ======================= BASE DE DATOS VIP =======================

class DatabaseVIP:
    """Base de datos premium del sistema"""
    
    def __init__(self, db_path: str = ConfiguracionVIP.DB_PATH):
        self.db_path = db_path
        self._init_database()
        self.logger = logging.getLogger(__name__)
    
    def _init_database(self):
        """Inicializa la base de datos premium"""
        try:
            import os
            ...
            # Tabla de partidos
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS partidos_vip (
                    id TEXT PRIMARY KEY,
                    liga TEXT,
                    pais TEXT,
                    local TEXT,
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS notificaciones_vip (
                    id TEXT PRIMARY KEY,
                    tipo TEXT,
                    partido_id TEXT,
                    mensaje TEXT,
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS estadisticas_vip (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    fecha DATE,
                    total_picks INTEGER,
                    picks_ganados INTEGER,
                    picks_perdidos INTEGER,
                    tasa_aciertos REAL,
                    roi_total REAL,
                    ligas_monitoreadas INTEGER,
                    partidos_hoy INTEGER,
                    partidos_en_vivo INTEGER
                )
            ''')
            ...
            self.logger.info("✅ Base de datos VIP inicializada")
            
        except Exception as e:
            self.logger.error(f"Error inicializando DB VIP: {e}")
...
class MonitorVIP:
    ...
    async def _crear_partido_vip(self, data: Dict, liga: Dict) -> Optional[PartidoVIP]:
        ...
        try:
            ...
            if mejor_cuota == 0 or mejor_cuota > self.reglas_usuario['cuota_inicial_maxima']:
                return None
    ...
    def _generar_mensaje_pre_partido(self, partido: PartidoVIP) -> str:
        """Genera mensaje previo al partido"""
    ...
    def _generar_mensaje_cambio_cuota(self, partido: PartidoVIP) -> str:
        """Genera mensaje de cambio de cuota"""
...
# ======================= SISTEMA DE INSIGHTS (CORREGIDO) =======================

class SistemaInsights:
    """Sistema de insights profesionales"""
